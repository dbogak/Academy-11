...
class Player:
    def __init__(
            self, surf, name="", x=random.randint(100, 400), y=random.randint(100, 400), mass=20,
            color=colors_players[random.randint(0, len(colors_players)-1)]):
    ...
    def draw(self, cam):
        ...
        y = cam.y
        pygame.draw.circle(
            self.surface, (col[0]-int(col[0]/3), int(col[1]-col[1]/3), int(col[2]-col[2]/3)),
            (int(self.x*zoom+x), int(self.y*zoom+y)), int((self.mass/2+3)*zoom)
        )
        pygame.draw.circle(
            self.surface, col, (int(self.x*cam.zoom+cam.x), int(self.y*cam.zoom+cam.y)), int(self.mass/2*zoom))
...
class Cell:
    ...
    def draw(self, cam):
        pygame.draw.circle(
            ...
            (int((self.x*cam.zoom+cam.x)), int(self.y*cam.zoom+cam.y)), int(self.mass*cam.zoom))
...
def draw_grid(camera):
    for i in range(0, 2001, 25):
        pygame.draw.line(surface, (230, 240, 240),
                         (0+camera.x, i*camera.zoom+camera.y), (2001*camera.zoom+camera.x, i*camera.zoom+camera.y), 3)
        pygame.draw.line(surface, (230, 240, 240),
                         (i*camera.zoom+camera.x, 0+camera.y), (i*camera.zoom+camera.x, 2001*camera.zoom+camera.y), 3)
